...
@Project  : fuguangapi
@Author   : QiaoPengjun
@Time     : 2023/10/17 上午9:42
@Software : PyCharm
@File     : tencentcloudapi.py
"""
import json
import logging
...
class TencentCloudAPI(object):
    ...
    def captcha(self, ticket, randstr, user_ip):
        ...
        try:
            ...
            params = {
                # 验证码类型固定为9
                "CaptchaType": int(Captcha["CaptchaType"]),
                # 客户端提交的临时票据
                "Ticket": ticket,
                # 客户端ip地址
                "UserIp": user_ip,
                ...
                "Randstr": randstr,
                # 验证码应用ID
                "CaptchaAppId": int(Captcha["CaptchaAppId"]),
                # 验证码应用key
            ...
            # 把响应结果转换成json格式数据
            # result--{'CaptchaCode': 1, 'CaptchaMsg': 'OK', 'EvilLevel': 0, 'GetCaptchaTime': 0, 'EvilBitmap': None, 'SubmitCaptchaTime': 1697540086, 'RequestId': '5b2457fd-a746-4cf7-b2d1-f5d61d683eb5'}
            result = json.loads(resp.to_json_string())
            return result and result.get("CaptchaCode") == 1

        except Exception as err:
            logging.error(f"TencentCloudAPI err--{err}")
